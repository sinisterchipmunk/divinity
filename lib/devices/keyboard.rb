module Devices
  class Keyboard < Devices::InputDevice
    ...
    def process_event(event)
      event_type, key_event = case event.type
        when SDL::KEYDOWN then [ :key_pressed,  Events::KeyPressed.new(event) ]
        when SDL::KEYUP   then [ :key_released, Events::KeyReleased.new(event) ]
        else raise "Unknown keyboard event type: #{event.type}"
      end
      
      engine.dispatch_event event_type, key_event
    end
