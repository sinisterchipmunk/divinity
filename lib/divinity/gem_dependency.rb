...
module Divinity
  class GemDependency < Gem::Dependency
    ...
    def self.unpacked_path
      @unpacked_path ||= File.join(DIVINITY_ROOT, 'vendor', 'gems')
    end
    ...
    def self.add_frozen_gem_path
      @@paths_loaded ||= begin
        source_index = Divinity::VendorGemSourceIndex.new(Gem.source_index)
        Gem.clear_paths
    ...
    def frozen?
      @frozen ||= vendor_divinity? || vendor_gem?
    end
    ...
    def build(options={})
      require 'divinity/gem_builder'
      return if specification.nil?
      if options[:force] || !built?
        return unless File.exists?(unpacked_specification_filename)
        spec = YAML::load_file(unpacked_specification_filename)
        Divinity::GemBuilder.new(spec, unpacked_gem_directory).build_extensions
    ...
    def refresh
      Divinity::VendorGemSourceIndex.silence_spec_warnings = true
      ...
      if frozen?
        ...
        if framework_gem?
          puts "Gem directory for #{name} not found - check if it's loading before divinity."
        else
    ...
    private
      ...
      def unpack_base
        Divinity::GemDependency.unpacked_path
      end
