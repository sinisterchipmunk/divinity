desc "List the gems that this divinity application depends on"
task :gems => 'gems:base' do
  Divinity.configuration.gems.each do |gem|
    print_gem_status(gem)
  end
  puts
  puts "I = Installed"
  puts "F = Frozen"
  puts "R = Framework (loaded before divinity starts)"
end
...
def current_gems
  gems = Divinity.configuration.gems
  gems = gems.select { |gem| gem.name == ENV['GEM'] } unless ENV['GEM'].blank?
  gems
end

def frozen_gems(load_specs=true)
  Dir[File.join(DIVINITY_ROOT, 'vendor', 'gems', '*-*')].map do |gem_dir|
    Divinity::GemDependency.from_directory_name(gem_dir, load_specs)
  end
end
