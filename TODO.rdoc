...
* Add an "interface" class method for engine controllers. Alternatively, an engine controller's view should accept an
  "interface do [setup interface] end" syntax. If both are supplied, it should evaluate the interface specified by the
  controller followed by the one in the view.

* Add generators for: interface, plugin, content_module, mod (same as content_module), model (resource)
